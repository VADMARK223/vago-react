...
export default function EventLoopChapter() {
  return (
    <>
      ...
      <p>
        JS в браузере и Node.js в основном <b>однопоточный</b> для твоего кода: в один момент
        времени выполняется только одна штука. Асинхронность достигается очередями и циклом
        обработки.
      </p>
      <hr />
      ...
      <ul>
        <li>
          таймеры (<code>setTimeout</code>)
        </li>
        <li>
          запросы (<code>fetch</code>)
        </li>
        <li>
          события (<code>click</code>)
        </li>
      ...
      <ul>
        ...
        <li>
          (ещё: <code>MutationObserver</code> в браузере)
        </li>
      </ul>
      <p>
        <code>Особенность</code>: микрозадачи выполняются <b>сразу после синхронного кода</b>, и{' '}
        <b>все подряд</b>, пока очередь не станет пустой.
      </p>
      ...
      <ul>
        ...
        <li>
          обработчики событий (<code>click</code>, <code>message</code>)
        </li>
        <li>некоторые I/O задачи</li>
      </ul>
      <p>
        <b>Особенность</b>: макрозадачи выполняются <code>по одной за “тик”</code> цикла.
      </p>
      ...
      <ol>
        <li>
          Выполни <b>весь синхронный код</b> (пока стек не пуст)
        </li>
        <li>
          Выполни <b>ВСЕ microtasks</b> (пока очередь не пустая)
        </li>
        <li>
          Выполни <b>ОДНУ macrotask</b>
        </li>
      ...
      <p></p>
      <p>
        <b>Sync → Microtasks → Macrotask → (Render) → Sync → ...</b>
      </p>
