...
export default function ReduceChapter() {
    return (
        <>
            <p><code>reduce</code> - это метод массива, который превращает его в <b>одно итоговое значение</b></p>
            <p>Это может быть:</p>
            <ul>
                <li>число</li>
                <li>строка</li>
            ...
            <hr/>
            <h2>Синтаксис</h2>
            <CodeBlock code={`array.reduce((accumulator, currentValue) => {
    return newAccumulator
}, initialValue)`}/>
            <p>Где:</p>
            <ul>
                <li><code>accumulator</code> — накопленное значение</li>
                <li><code>currentValue</code> — текущий элемент массива</li>
                <li><code>initialValue</code> — стартовое значение</li>
            </ul>
            <hr/>
...
console.log(sum) // 10`}/>
            ...
            <CodeBlock code={`type ChapterType = 'react' | 'ts' | 'js'
...
const grouped = chapters.reduce<Record<ChapterType, Chapter[]>>(
    ...
    { react: [], ts: [], js: [] }
            ...
            <CodeBlock code={`numbers.reduce((acc, num) => acc + num)
`}/>
            <p>Тогда:</p>
            <ul>
