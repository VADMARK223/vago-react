...
export function InterfacesAndTypes() {
    return (
        <>
            ...
            <h3 id={'rules'}>Правило №1 — Props = <code>interface</code></h3>
            <p>Всегда описываем props компонентов через <code>interface</code>.</p>
            <CodeBlock code={`interface CodeBlockProps {
  code: string
  lang?: CodeLang
}
...
export function CodeBlock({ code, lang = 'tsx' }: CodeBlockProps) {}`}/>
            ...
            <CodeBlock code={`type CodeLang = 'tsx' | 'ts' | 'js' | 'go'
type Theme = 'light' | 'dark'
type Size = 'sm' | 'md' | 'lg'`}/>
            <hr/>
            <h3 id={'rules'}>Правило №3 — DTO / API / утилитарные типы = type</h3>
            <CodeBlock code={`type UserDto = {
  id: string
  email: string
}
...
type UserPreview = Pick<UserDto, 'id' | 'email'>`}/>
            ...
            <CodeBlock code={`interface BaseButtonProps {
  disabled?: boolean
}
...
interface IconButtonProps extends BaseButtonProps {
  icon: ReactNode
            ...
            <p>❌ плохо:</p>
            <CodeBlock code={`type ButtonProps = { onClick: () => void }
