...
export function UseReducerBlock() {
    return (
        <>
            ...
            <p>Хук для управления <b>более сложным состоянием</b>, когда <code>useState</code> уже начинает путаться и перегружен.</p>
            <p>useReducer — это альтернатива <code>useState</code>, основанная на идее редьюсера (как в Redux):</p>
            <blockquote>есть state → action → reducer → новый state</blockquote>
            <p>Он особенно полезен, когда:</p>
            <ul>
                <li>состояние <b>объект или вложенная структура</b></li>
            ...
            <ul>
                <li><code>state</code> — текущее состояние</li>
                <li><code>dispatch(action)</code> — отправка действия</li>
                <li><code>reducer(state, action)</code> — чистая функция, которая возвращает новый state</li>
            </ul>
            <hr/>
